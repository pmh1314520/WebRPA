...
"校验哈希链完整性":"Verify hash-chain integrity","共 ":"Total ","条审计记录":" audit records","暂无审计记录":"No audit records","链完整，共 ":"Chain intact, total ","条":" records","检测到篡改！断裂于 #":"Tampering detected! Broken at #","导出CSV":"Export CSV","导出JSONL":"Export JSONL","已导出":"Exported","导出失败":"Export failed",
...
async function vAudit(){
 ...
 let h='<div class="inline" style="margin-bottom:8px"><button class="act gray" onclick="verifyChain()">校验哈希链完整性</button><button class="act gray" onclick="exportAudit(\'csv\')">导出CSV</button><button class="act gray" onclick="exportAudit(\'jsonl\')">导出JSONL</button><span class="mut">共 '+(st.total||0)+' 条审计记录</span></div>';
