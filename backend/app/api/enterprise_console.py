...
async function vApprovals(){
 ...
 let h='<div class="card">'+(reqs.length?reqs.map(q=>{
  ...
  return '<div class="row">'+statusTag(q.status)+'<div class="flex1"><div class="title">'+esc(q.action)+' <span class="mut">'+esc(q.target)+'</span></div><div class="mut">发起 '+esc(q.requester)+' · '+esc(q.created_at)+(q.reason?(' · '+esc(q.reason)):'')+(q.approver?(' · 审批人 '+esc(q.approver)):'')+'</div></div>'+btn+'</div>'}).join(''):'<div class="empty">暂无审批单</div>')+'</div>';
 setView(h);
}
