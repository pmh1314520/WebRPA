...
"在线节点":"Online nodes","负载/容量":"Load/Capacity","利用率":"Utilization","运行中任务":"Running tasks","派发集群任务":"Dispatch cluster task","工作流文件名":"Workflow file name","标签约束（逗号分隔，可空）":"Tag constraints (comma-separated, optional)","能力约束（逗号分隔，可空）":"Capability constraints (comma-separated, optional)","派发":"Dispatch","执行机节点":"Robot nodes","暂无执行机。执行机可调用 /api/orchestrator/nodes/register 注册":"No robots yet. Robots register via /api/orchestrator/nodes/register","集群任务":"Cluster tasks","暂无任务":"No tasks","移除":"Remove","负载 ":"Load ","标签[":"tags[","能力[":"caps[","节点 ":"node ","尝试 ":"attempts ","已派发：":"Dispatched: ","请填写工作流文件名":"Please enter a workflow file name","确定移除该节点？":"Remove this node?","已移除":"Removed","暂无执行机。在执行机上运行 cluster_node.py 即可加入集群":"No robots yet. Run cluster_node.py on an execution machine to join the cluster",
...
"校验哈希链完整性":"Verify hash-chain integrity","共 ":"Total ","条审计记录":" audit records","暂无审计记录":"No audit records","链完整，共 ":"Chain intact, total ","条":" records","检测到篡改！断裂于 #":"Tampering detected! Broken at #","导出CSV":"Export CSV","导出JSONL":"Export JSONL","已导出":"Exported","导出失败":"Export failed","，已归档 ":", archived ","个文件":" files",
...
"活动会话（":"Active sessions (","强制下线":"Force logout","暂无活动会话":"No active sessions","强制下线用户 ":"Force logout all sessions of "," 的所有会话？":"?","已吊销 ":"Revoked ","个会话":" sessions",
...
async function vAudit(){
 ...
 let arch=[];try{arch=(await api('/audit/archives')).archives||[]}catch(e){}
 let h='<div class="inline" style="margin-bottom:8px"><button class="act gray" onclick="verifyChain()">校验哈希链完整性</button><button class="act gray" onclick="exportAudit(\'csv\')">导出CSV</button><button class="act gray" onclick="exportAudit(\'jsonl\')">导出JSONL</button><span class="mut">共 '+(st.total||0)+' 条审计记录'+(arch.length?('，已归档 '+arch.length+' 个文件'):'')+'</span></div>';
...
async function exportAudit(fmt){try{const r=await fetch('/api/audit/export?fmt='+fmt,{headers:{'x-webrpa-session':session}});if(!r.ok){toast('导出失败');return}const blob=await r.blob();const url=URL.createObjectURL(blob);const a=document.createElement('a');a.href=url;a.download='audit_log.'+fmt;document.body.appendChild(a);a.click();a.remove();URL.revokeObjectURL(url);toast('已导出')}catch(e){toast(e.message)}}
...
async function vSettings(){
 ...
 let sessions=[];try{sessions=(await api('/rbac/sessions')).sessions||[]}catch(e){}
 h+='<div class="card"><h3 style="margin-top:0">活动会话（'+sessions.length+'）</h3>'+(sessions.length?sessions.map(s=>'<div class="row"><div class="flex1"><div class="title">'+esc(s.username)+' <span class="mut">…'+esc(s.token_tail)+'</span></div></div><button class="act bad" onclick="revokeSessions(\''+esc(s.username)+'\')">强制下线</button></div>').join(''):'<div class="empty">暂无活动会话</div>')+'</div>';
 setView(h);
}
async function revokeSessions(u){if(!confirm('强制下线用户 '+u+' 的所有会话？'))return;try{const r=await api('/rbac/sessions/revoke',{method:'POST',body:JSON.stringify({username:u})});toast('已吊销 '+r.revoked+' 个会话');refresh()}catch(e){toast(e.message)}}
