...
<p align="center">
  <img src="https://img.shields.io/badge/版本-2.5.0-blue.svg" alt="版本">
...
项目内置 558 个功能模块与一位具备自愈能力的 AI 小助手：你可以用一句话描述需求，由 AI 自动搭建、排错并重跑工作流。所有运行环境（Python 3.13、Node.js、浏览器引擎、媒体与文档处理工具）均已随包内置，解压双击启动器即可使用，无需联网、无需额外安装，天然适配内网与局域网部署。
...
- **模块丰富**：558 个功能模块，覆盖 95% 自动化场景（含基于 DrissionPage 的反检测网页自动化模块）
...
依托 558 个内置模块（按场景分门别类，拖入画布即可使用），WebRPA 覆盖以下关键能力：
...
图形化启动器基于 Electron，自带 Chromium，无需安装系统 WebView2 运行库，开箱即用。如果不想使用图形化启动器，或启动器无法运行，可使用随包附带的 **bat 备用启动脚本**，同样能把前后端全部服务跑起来：

- 双击运行 `WebRPA备用启动.bat`：自动校验运行环境、读取端口配置、（首次运行）自动安装前端依赖，并在两个独立窗口分别启动后端与前端，就绪后自动打开浏览器。
- 需要停止服务时，运行 `WebRPA停止服务.bat` 即可按端口一键结束前后端进程。

> bat 脚本与图形化启动器的启动效果完全一致，二者任选其一即可。
